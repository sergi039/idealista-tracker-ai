...
class SearchProfile(db.Model):
    ...
    classification_rules = db.Column(JSON)  # [{"category": "...", "subtype": "...", "pattern": "...", "priority": 50}, ...]
    travel_targets = db.Column(JSON)  # list of targets (schema in SettingsService/SearchProfileService)
    ...
    ai_config = db.Column(JSON)  # optional AI prompt/context overrides (e.g., market_context)
...
class Property(db.Model):
    ...
    __table_args__ = (
        ...
        CheckConstraint('price IS NULL OR price >= 0', name='ck_properties_price_non_negative'),
        CheckConstraint('area IS NULL OR area >= 0', name='ck_properties_area_non_negative'),
    ...
    search_profile_id = db.Column(db.Integer, db.ForeignKey('search_profiles.id', ondelete='SET NULL'), nullable=True)
    search_profile = db.relationship('SearchProfile', backref=db.backref('properties', lazy='dynamic'))

    title = db.Column(db.Text)
    url = db.Column(db.Text)

    # Deal & type classification (config-driven; no hard constraints here by design).
    deal_type = db.Column(db.String(16), default='sale')  # 'sale' | 'rent' (rent not used initially)
    property_category = db.Column(db.String(32))  # e.g. 'housing', 'land', 'garage', 'commercial', 'building'
    property_subtype = db.Column(db.String(64))  # e.g. 'apartment', 'house', 'penthouse', 'plot', ...

    price = db.Column(db.Numeric(10, 2))
    currency = db.Column(db.String(3), default='EUR')
    area = db.Column(db.Numeric(10, 2))
    area_type = db.Column(db.String(16), default='unknown')  # 'built' | 'plot' | 'unknown'
    ...
    property_details = db.Column(JSON)  # Raw/detail blocks (kept for compatibility patterns)
    ...
    def to_dict(self):
        return {
            ...
            'price_changed_date': self.price_changed_date.isoformat() if self.price_changed_date else None,
...
class Land(db.Model):
    ...
    updated_at = db.Column(db.DateTime, default=utcnow, onupdate=utcnow)  # Last update time
...
class PropertyAiAnalysisVariant(db.Model):
    ...
    property = db.relationship('Property', backref=db.backref('ai_analysis_variants', lazy='dynamic'))

    __table_args__ = (
        db.Index('ix_property_ai_analysis_variants_property_provider', 'property_id', 'provider'),
    )
