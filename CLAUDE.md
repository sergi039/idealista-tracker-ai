...
- **The text claim asks Jev first, and the bridge decides whatever Jev declines** (2026-09-19): `classify_text_with_ai` → `classify_text_with_jev` (one typed Choice through `services/typesafe_transport.py`, the app's one per-token-billed route; `TYPESAFE_API_KEY` lives only in the mini's `.env`) then `classify_text_with_bridge`; below `SEA_VIEW_TEXT_MIN_CONFIDENCE` (0.7), without a key or on any failure the bridge answers as before; the model is pinned (`TYPESAFE_MODEL` = `jev-1.13.0`), a confidence must be a finite number in [0, 1] and is compared before rounding, research notes (`RESEARCH_NOTES_PREFIX`) never reach Jev, and the Enrich allowance adds twice the Jev timeout (connect plus a read deadline) to the bridge's; the detail records `provider` / `confidence` / `model` / `jev_status` / `jev_error` / `jev_claim` / `jev_confidence` / `jev_ms` and a text fingerprint, on every attempt; nothing else may read the key; ingestion (`use_ai=False`) reaches neither model; a time-boxed experiment, dates and exit criteria in PR #574. → docs/rules/sea-view.md
...
- **A ROOFTOP does not earn its zero on this coast, and the cadastre says so for free** (#559): `services/cadastre_locate.py` asks Catastro which parcel lies under a coordinate (`Consulta_RCCOOR_Distancia`, one keyless request) and, for a row it cannot place that way, where the asked address actually is (`ObtenerCallejero` + `Consulta_DNPLOC` + the parcel's own reference point); `utils/audit_precise_against_cadastre.py` runs it and **writes nothing**. Measured over all 130 rows on 2026-09-08: 58 stand on the parcel numbered as asked, 26 displacements are measured (nine of them 62–1783 m) and 46 rows are neither confirmed nor placed — so `_tier_slack_table` is deliberately unchanged, because a band over the half that resolved is not a band. A matched parcel whose street shares no content word with the street asked is `number_matched_other_street`, never an agreement. → docs/rules/coordinates.md
- **The seven rows #535 left behind carry a person's coordinate now** (#558): six were established from Catastro and three from the portal's own map centre, which is a ~500 m privacy circle and is labelled as one; 1445 moved 3.3 km once its mangled title was read as the lugar `VILACENDOI - SAN MARTIÑO`; 438 and 765 keep their coordinate and lose a `precise` no query ever earned. → docs/rules/coordinates.md
...
- **The portal's own photographs are captured on the way past and rendered on three surfaces** (#548, #550): `services/portal_photos.py` is the one reader (`photos_for`); a URL carrying a credential is refused wherever it appears, what the payload NAMED is stored beside what was captured, bytes are never stored, and the model never sees a pixel -- the bridge is text-only, so the owner is the sensor.
...
- **A coordinate a person established outranks the geocoder, in its own key** (GEO-002): `enrichment["location"]`, written by `utils/set_property_location.py` with a required note and refused before the geocode; never written where `portal_coordinate` looks; the ad-hoc rows are converted by hand, never backfilled (161 and 792 done, #536); the tool's `MISFILED` line fires on a hand-set row too and knows a portal's pin by the portal's name, not by one spelling; `utils/refresh_property_accuracy.py` counts and names what it skipped.
...
- **The prompt carries the parcel, and for a while it silently did not** (#547, 2026-09-04): `gather_facts` asked for `cadastre["metrics"]["bbox_fill"]` while the service writes `cadastre["geometry"]["bbox_fill_ratio"]`, so `CADASTRAL PARCEL` and `PARCEL SHAPE` had reached no prompt at all; both ratios are fed with each one's blind spot named and no mapping from a number to a verdict; the values are total and fail-closed (`_finite`), Catastro's own words are collapsed to one line (`_one_line`), and a row with no parcel still says nothing — the one place in this module where #98's rule is deliberately not applied.
...
- **The bridge falls back to the owner's other Claude profile on an account refusal, never on a request's** (2026-09-16): `AI_BRIDGE_CLAUDE_FALLBACK_CONFIG_DIRS` in the launcher's `.env`, `default` for the CLI's own profile; a 429 / weekly limit / not-logged-in moves on, a 400 does not; the answer carries `claude_profile`; codex is one account and has no fallback (`tests/test_ai_bridge_claude_profiles.py`). → docs/rules/ai-bridge.md
