...
class EmailParser:
    ...
    def parse_idealista_email(self, email_content: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if (
                extracted_data["url"]
                or extracted_data["title"]
                or extracted_data["price"]
            ):
    ...
    def _extract_title(self, text: str) -> str:
        ...
        location_match = re.search(
            r"([A-ZÁÉÍÓÚ][a-záéíóúñ\s-]+)\s*,\s*([A-ZÁÉÍÓÚ][a-záéíóúñ\s-]+)",
            text,
            re.IGNORECASE,
        )
        if location_match:
    ...
    def _extract_area(self, text: str) -> Optional[float]:
        ...
        # Collect every match from every pattern (instead of first-pattern-
        # wins) and prefer the longest leftmost match: with the patterns now
        # anchored, a longer match at the same position captures the whole
        # number rather than a trailing fragment of it.
        candidates = []
        for pattern in self.patterns["area"]:
            for match in re.finditer(pattern, text, re.IGNORECASE):
                ...
                # parser is the legacy land-only pipeline); reject anything
                # smaller as a parse artifact rather than a real plot size.
                if area < 100:
                    continue
                candidates.append((match.start(), -(match.end() - match.start()), area))
    ...
    def _extract_municipality(self, text: str) -> Optional[str]:
        ...
        if title_match:
            ...
            if title_municipality:
                logger.debug(
                    f"Extracted municipality from title: '{title_municipality}'"
                )
        ...
        if land_match:
            ...
            if "your search" not in location.lower():
                ...
                if location and len(location) > 2:
                    municipality = self._extract_municipality_from_title(
                        f"Land in {location}"
                    )
                    if municipality:
                        logger.debug(
                            f"Extracted municipality from 'Land in': '{municipality}'"
                        )
                        return municipality
        ...
        for i, pattern in enumerate(self.patterns["municipality"]):
            ...
            if match:
                ...
                if self._is_valid_municipality(municipality):
                    logger.debug(
                        f"Extracted municipality from pattern {i}: '{municipality.title()}'"
                    )
                    return municipality.title()
                else:
                    logger.debug(
                        f"Rejected municipality '{municipality}' - failed validation"
                    )
    ...
    def _extract_municipality_from_title(self, title: str) -> Optional[str]:
        ...
        prefix_match = re.match(
            r"^(land in|terreno en|parcela en|solar en)\s+", raw, re.IGNORECASE
        )
        if prefix_match:
            raw = raw[prefix_match.end() :].strip()

        raw = re.sub(
            r"\s*\d[\d\.,]*\s*(?:€|eur|&euro;|â‚¬).*$", "", raw, flags=re.IGNORECASE
        ).strip()
    ...
    def _classify_land_type(self, text: str) -> Optional[str]:
        ...
        if any(word in text_lower for word in ["solar", "parcela", "terreno"]):
            if any(
                word in text_lower for word in ["construir", "edificar", "vivienda"]
            ):
                return "buildable"
    ...
    def _clean_description(self, body: str) -> str:
        ...
        if "Hello" in description:
            ...
            hello_match = re.search(
                r"Hello.*?(?:Contact us|See all listings from|From Your searches|With the idealista app)",
                description,
                re.DOTALL,
            )
            if hello_match:
        ...
        description = re.sub(
            r"Does this listing.*", "", description, flags=re.IGNORECASE | re.DOTALL
        )
        description = re.sub(
            r"From Your searches.*", "", description, flags=re.IGNORECASE
        )
        description = re.sub(
            r"With the idealista app.*", "", description, flags=re.IGNORECASE
        )
        description = re.sub(
            r"If you.re no longer interested.*", "", description, flags=re.IGNORECASE
        )
    ...
    def parse_no_longer_listed_email(self, email_content: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if not any(
                p in subject.lower() or p in body.lower() for p in no_longer_patterns
            ):
                return None
