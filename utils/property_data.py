...
def normalize_property_details(property_details: Any) -> Dict:
    ...
    try:
        ...
        if isinstance(property_details, str):
            try:
                ...
                if isinstance(parsed, dict):
                    ...
                    logger.warning(
                        f"Property details JSON parsed to non-dict type: {type(parsed)}"
                    )
                    return {}
        ...
        logger.warning(
            f"Property details has unexpected type: {type(property_details)}"
        )
        return {}
...
def ensure_property_details_dict(land) -> None:
    ...
    try:
        ...
        logger.error(
            f"Error ensuring property_details dict for land {getattr(land, 'id', 'unknown')}: {e}"
        )
        land.property_details = {}


def update_property_details_section(
    property_details: Dict, section_key: str, section_data: Dict
) -> Dict:
    """
