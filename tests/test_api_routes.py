...
class TestManualIngestion:
    ...
    def _this_machine_ingests(self, app):
        ...
        # AUTO_START_SCHEDULER, so the fail-closed default applies (#376). Set
        # through app.config, the source both the scheduler and the guard read.
...
class TestPropertyStatus:
    def test_set_property_status_success(self, client, app):
        ...
        response = client.post(
            f"/api/property/{prop_id}/set-status", json={"status": "removed"}
        )
        assert response.status_code == 200
        data = json.loads(response.data)
        assert data["success"] is True
    ...
    def test_set_property_status_invalid(self, client, app):
        ...
        response = client.post(
            f"/api/property/{prop_id}/set-status", json={"status": "bad_status"}
        )
...
class TestPropertiesCSVExport:
    def test_export_properties_csv(self, client, app):
        ...
        assert "idealista_properties.csv" in response.headers.get(
            "Content-Disposition", ""
        )
...
class TestPropertiesAPI:
    def test_get_properties_defaults_to_default_profile(self, client, app):
        with app.app_context():
            ...
            p1 = Property(
                source_email_id="api_prop_1",
                title="A",
                municipality="Madrid",
                search_profile_id=default_profile.id,
            )
            p2 = Property(
                source_email_id="api_prop_2",
                title="B",
                municipality="Madrid",
    ...
    def test_get_properties_filter_and_hide_removed(self, client, app):
        ...
        response2 = client.get(
            "/api/properties?category=housing&subtype=apartment&hide_removed=0"
        )
        assert response2.status_code == 200
        data2 = json.loads(response2.data)
        assert data2["success"] is True
    ...
    def test_get_property_detail(self, client, app):
        with app.app_context():
            ...
            prop = Property(
                source_email_id="api_prop_detail_1",
                title="Detail",
                municipality="Madrid",
...
class TestCriteriaAPI:
    ...
    def test_update_criteria_missing_data(self, client):
        ...
        response = client.put(
            "/api/criteria", data=json.dumps({}), content_type="application/json"
        )
    ...
    def test_update_criteria_service_failure(self, mock_scoring_service, client):
        ...
        criteria = {"criteria": {"infrastructure_basic": 0.25}}

        response = client.put(
            "/api/criteria", data=json.dumps(criteria), content_type="application/json"
        )
...
class TestAPIErrorHandling:
    ...
    def test_invalid_json(self, client):
        ...
        response = client.put(
            "/api/criteria", data="invalid json", content_type="application/json"
        )
...
class TestUniversalPropertyAI:
    ...
    def test_property_ai_openai_does_not_overwrite_claude_field(
        self, mock_analyze, client, app
    ):
        with app.app_context():
            prop = Property(
                source_email_id="prop_ai_openai_1",
                title="Test Property",
            ...
            resp = client.post(
                f"/api/property/{prop.id}/analyze/structured",
            ...
            variant = (
                PropertyAiAnalysisVariant.query.filter_by(
                    property_id=prop.id, provider="openai"
                )
    ...
    @patch("services.property_ai_service.PropertyAIService.analyze_property_structured")
    def test_property_ai_claude_updates_property_ai_analysis(
        self, mock_analyze, client, app
    ):
        with app.app_context():
            prop = Property(
                source_email_id="prop_ai_claude_1",
                title="Test Property",
                municipality="Alicante",
            )
            db.session.add(prop)
            ...
            variant = (
                PropertyAiAnalysisVariant.query.filter_by(
                    property_id=prop.id, provider="claude"
                )
                .order_by(PropertyAiAnalysisVariant.created_at.desc())
                .first()
            )
    ...
    def test_property_ai_compare_endpoint(self, client, app):
        with app.app_context():
            prop = Property(
                source_email_id="prop_ai_compare_1",
                title="Test Property",
                municipality="Alicante",
            )
            prop.ai_analysis = {
                "rental_market_analysis": {
                    "investment_rating": "GOOD - Above average returns"
                }
            }
            db.session.add(prop)
            db.session.commit()
            ...
            db.session.add(
                PropertyAiAnalysisVariant(
                    ...
                    analysis={
                        "rental_market_analysis": {
                            "investment_rating": "MODERATE - Standard market returns"
                        }
                    },
                )
            )
    ...
    @patch(
        "services.property_enrichment_service.PropertyEnrichmentService.enrich_property",
        return_value=True,
    )
    def test_property_google_enrich_endpoint(self, mock_enrich, client, app):
        with app.app_context():
            prop = Property(
                source_email_id="prop_enrich_1",
                title="Test Property",
                municipality="Alicante",
            )
            db.session.add(prop)
