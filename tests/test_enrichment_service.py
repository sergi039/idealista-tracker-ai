...
class TestEnrichmentService:
    ...
    def test_init(self, enrichment_service):
        ...
        assert (
            enrichment_service.osm_overpass_url
    ...
    def test_enrich_land_no_coordinates(self, mock_scoring, app, enrichment_service):
        ...
        with app.app_context():
            ...
            with patch.object(
                enrichment_service.geocoding_service, "geocode_address"
            ) as mock_geocode:
    ...
    def test_geocode_with_accuracy_uses_title_parts_without_municipality(
        self, app, enrichment_service
    ):
        ...
        with app.app_context():
            ...
            with patch.object(
                enrichment_service,
                "_extract_municipality_from_title",
                return_value=None,
            ):
                with patch.object(
                    enrichment_service.geocoding_service,
                    "geocode_address",
                ...
                "results": [
                    {
                        ...
                        "types": ["supermarket"],
                        "geometry": {"location": {"lat": 39.4700, "lng": -0.3760}},
                    }
                ]
            }
    ...
    def test_enrich_with_google_places_no_api_key(
        self, app, enrichment_service, test_land
    ):
    ...
    @patch("services.enrichment_service.request_with_retries")
    def test_enrich_with_osm_data_success(
        self, mock_post, app, enrichment_service, test_land
    ):
        ...
        with app.app_context():
            ...
            mock_response.json.return_value = {
                "elements": [
                    {"tags": {"amenity": "supermarket", "name": "OSM Supermarket"}},
                    {"tags": {"amenity": "school", "name": "OSM School"}},
                ]
            }
    ...
    def test_analyze_environment_mountain_view(
        self, app, enrichment_service, test_land
    ):
