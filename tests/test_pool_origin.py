...
class TestPropertyPageNote:
    """Read-only wiring on `templates/property_detail.html` (#346, scope
    item 2): a muted note only when the state is `differs`, nothing that
    scores or acts on it."""

    def test_the_note_renders_when_the_origin_differs(self, app, client):
        prop = _prop(
            title="page-differs",
            location_lat=43.9,
            location_lon=-7.5,
            enrichment={
                ...
                "pool": {
                    "status": "ok",
                    "origin": {"lat": 43.55, "lon": -6.83},
                    "candidates": [{"name": "Piscina Municipal", "drive_min": 12}],
                },
            },
        )
        body = client.get(f"/properties/{prop.id}").get_data(as_text=True)
        assert "Measured at a different coordinate" in body

    def test_no_note_when_the_origin_matches(self, app, client):
        prop = _prop(
            title="page-matches",
            enrichment={
                "quality_of_life": {"municipality": {"status": "not_matched"}},
        ...
        body = client.get(f"/properties/{prop.id}").get_data(as_text=True)
        assert "Measured at a different coordinate" not in body

    def test_no_note_for_a_legacy_block_with_no_origin(self, app, client):
        prop = _prop(
            title="page-legacy",
            enrichment={
                "quality_of_life": {"municipality": {"status": "not_matched"}},
