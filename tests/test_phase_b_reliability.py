    ...
    with app.app_context():
        land = Land(
            ...
            infrastructure_basic={
                ...
                'internet': True, 'gas': True,
            },
            environment={
                'sea_view': True, 'mountain_view': True,
                'forest_view': True, 'orientation': 'south',
            },
        )
        db.session.add(land)
        db.session.commit()
...
class TestScoreClamping:
    ...
    def test_oversized_subscores_clamped_to_100(self, app, test_land):
        ...
        with app.app_context():
            svc = ScoringService()
            land = db.session.get(Land, test_land)
            with patch.object(svc, '_score_location_quality', return_value=250), \
...
class TestSchedulerLockCleanup:
    ...
    def test_lock_released_on_init_failure(self, app):
        ...
        with app.app_context():
            ...
            with patch.object(app.config, 'get', side_effect=lambda k, d=None: {
                'TESTING': False,
            }.get(k, d)):
                with patch('services.scheduler_service.Config') as mock_config:
                    ...
                    with patch('services.scheduler_service.BackgroundScheduler',
...
class TestDistanceMatrixValidation:
    ...
    def test_batch_missing_fields_skipped(self):
        ...
        api_response = {
            ...
            'rows': [{
                'elements': [
                    {'status': 'OK', 'duration': {'value': 600}, 'distance': {'value': 10000}},
