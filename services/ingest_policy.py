...
It deliberately introduces no new flag. Adding `INGEST_ENABLED` would mean a
second thing to set, a second thing to forget, and a machine could end up
declaring the two halves of one fact differently. The question "is this the
machine that ingests?" already has an answer in the configuration; this module
gives it one name and two readers — the endpoint and the navbar — so the button
is absent exactly where the endpoint would refuse.
...
def ingest_verdict() -> IngestVerdict:
    """Read the machine's ingester role from configuration.

    Imported inside the function on purpose: `Config` reads the environment at
    import time, and tests (plus `docker compose run` siblings) set the flag
    after this module is already imported.
    """
    from config import Config

    if bool(getattr(Config, "AUTO_START_SCHEDULER", False)):
