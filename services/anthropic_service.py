...
class AnthropicService:
    ...
    def __init__(self):
        ...
        if not Config.AI_BRIDGE_TOKEN:
            logger.error(
                "AI_BRIDGE_TOKEN is not set - the subscription bridge is unconfigured"
            )
            raise ValueError(
...
Format your response in clear sections."""
            ...
            return {"analysis": None, "error": "AI analysis failed", "status": "failed"}
...
{description}"""
    ...
    def find_similar_properties(
        self, current_property: Dict[str, Any], limit: int = 3
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            query = query.order_by(
                Land.score_total.desc().nullslast(), Land.created_at.desc()
            )

            # Get similar properties
            similar_properties = query.limit(
                limit * 2
            ).all()  # Get more to filter later
            ...
            for prop in similar_properties:
                ...
                if prop.id == current_id:
                    logger.debug(
                        f"Skipping current property {current_id} in similar results"
                    )
                    continue
                ...
                if similarity_score > 0:  # Only include if there's some similarity
                    prop_dict = {
                        ...
                        "score_total": float(prop.score_total)
                        if prop.score_total
                        else None,
                    ...
                    logger.debug(
                        f"Added similar property ID: {prop.id} with score: {similarity_score:.2f}"
                    )
    ...
    def _calculate_similarity_score(self, current: Dict[str, Any], other) -> float:
        ...
        if current_price and other_price and current_price > 0 and other_price > 0:
            price_ratio = min(current_price, other_price) / max(
        ...
        try:
            ...
            if MarketAnalysisService:
                try:
                    ...
                    land = (
                        db.session.query(Land)
                        .filter_by(id=property_data.get("id"))
                        .first()
                    )
                    if land:
            ...
            if similar_properties:
                ...
                for i, prop in enumerate(similar_properties, 1):
                    similar_text += (
                        f"\n{i}. {prop['title'][:80]}... - €{prop['price']:,.0f} - {prop['area']:.0f}m² - {prop['municipality']} - Score: {prop['score_total']:.1f}/100"
                        if prop["price"] and prop["area"] and prop["score_total"]
            ...
            if is_enrichment:
                ...
                existing_sections = (
                    list(existing_analysis.keys()) if existing_analysis else []
                )
                missing_sections = []
                incomplete_sections = []
...
Provide ONLY the missing or enhanced data in this EXACT JSON format (keep all text in English). Include ALL sections but focus enrichment on priority areas:"""
                ...
                try:
                    ...
                    profile_id = property_data.get(
                        "search_profile_id"
                    ) or property_data.get("profile_id")
...
Keep all responses concise and in English. Focus on practical investment insights."""
            ...
            logger.error(
                "Failed to analyze property structure with Claude", exc_info=True
            )
            ...
            return {"error": user_msg, "status": "failed"}
    ...
    def _format_comprehensive_data(self, property_data: Dict[str, Any]) -> str:
        ...
        if property_data.get("travel_time_nearest_beach"):
            text_parts.append(
                f"BEACH ACCESS: {property_data['travel_time_nearest_beach']} min to {property_data.get('nearest_beach_name', 'beach')}"
            )
        try:
