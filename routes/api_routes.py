...
@api_bp.route('/land/<int:land_id>/scrape-idealista', methods=['POST'])
def scrape_idealista_details(land_id):
    """Manually trigger Idealista web scraping for a specific property"""
    try:
        ...
        # Use WebScraperService validation instead of simple substring check
        # This supports both .com and .es domains as configured in the scraper
        web_scraper = WebScraperService()
        if not land.url or not web_scraper._validate_url(land.url):
            return jsonify({
                "success": False,
                "error": "Property does not have a valid Idealista URL"
            }), 400
        
        logger.info(f"Manual Idealista scraping requested for land {land_id}")
        
        scraped_data = web_scraper.scrape_property_details(land.url)
        
        if not scraped_data:
            return jsonify({
                "success": False,
                "error": "Failed to scrape data from Idealista URL"
            }), 500
        
        # Use unified property_details normalization helper
        ...
        return jsonify({
            "success": True,
            "message": f"Successfully scraped Idealista data for property {land_id}",
            "status": scraped_data['meta']['status'],
            "data": scraped_data
        })
        
    except Exception as e:
        logger.error(f"Manual Idealista scraping failed for land {land_id}: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
